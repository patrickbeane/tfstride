...
class CliTests(unittest.TestCase):
    ...
    def test_cli_fail_on_threshold_still_writes_output_file(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn("Policy gate failed", stderr_buffer.getvalue())
            self.assertIn("8 medium", stderr_buffer.getvalue())
    ...
    def test_cli_provider_option_can_select_gcp_inventory(self) -> None:
        ...
        self.assertEqual(report["summary"]["active_findings"], 21)
            ...
            config_path.write_text(
                "\n".join(
                    [
                        ...
                        "",
                        "[rules]",
                        'disable = ["aws-private-data-transitive-exposure", "aws-workload-s3-vpc-endpoint-missing"]',
                        "",
