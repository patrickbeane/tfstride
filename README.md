...
Privileged identity assignment posture is normalized into a shared provider-neutral vocabulary for evidence and parity tests, while findings stay provider-owned so AWS IAM, GCP IAM, and Azure RBAC semantics remain separate. Workload secret access paths remain provider-local: AWS distinguishes ECS execution-role delivery from task-role permissions, GCP connects Cloud Run service accounts to exact Secret Manager grants, and Azure connects App Service Key Vault references to exact managed identities and vault authorization. Unknown, external, conditional, or computed access remains uncertainty rather than an unsafe claim, and secret values are never retained in evidence or reports.
