...
Privileged identity assignment posture is normalized into a shared provider-neutral vocabulary for evidence and parity tests, while findings stay provider-owned so AWS IAM, GCP IAM, and Azure RBAC semantics remain separate. Workload secret access paths remain provider-local: AWS distinguishes ECS execution-role delivery from task-role permissions and connects public ECS services to exact Secrets Manager grants, GCP connects public Cloud Run services to exact Secret Manager grants, and Azure connects public App Service Key Vault references to exact managed identities and vault authorization. Public exposure findings require deterministic workload exposure and unconditional modeled access; unknown, external, conditional, denied, or computed paths remain uncertainty rather than an unsafe claim, and secret values are never retained in evidence or reports.
