...
class AzureResourceDecorator:
    """Resolve Azure storage relationships and derive effective public posture."""

    def decorate(self, resources: list[NormalizedResource]) -> None:
        accounts_by_reference = _storage_accounts_by_reference(resources)
        self._apply_standalone_network_rules(resources, accounts_by_reference)
        self._derive_account_network_posture(resources)
        self._resolve_containers(resources, accounts_by_reference)

    def _apply_standalone_network_rules(
        self,
        resources: list[NormalizedResource],
        accounts_by_reference: dict[str, NormalizedResource],
    ) -> None:
        for network_rules in resources:
            ...
            reference = facts.storage_account_reference
            account = accounts_by_reference.get(_reference_key(reference))
            if account is None:
                facts.add_unresolved_storage_account_reference(reference)
                continue
...
def _storage_accounts_by_reference(
    resources: list[NormalizedResource],
) -> dict[str, NormalizedResource]:
    accounts: dict[str, NormalizedResource] = {}
    for resource in resources:
        ...
        for reference in (
            ...
            if key:
                accounts.setdefault(key, resource)
    return accounts
