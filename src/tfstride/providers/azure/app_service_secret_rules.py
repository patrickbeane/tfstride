...
class AzureAppServiceSecretDeliveryRuleDetectors:
    def __init__(self, finding_factory: FindingFactory) -> None:
        self._finding_factory = finding_factory

    def detect_inline_sensitive_app_setting(
        self,
        ...
        for app in context.inventory.by_type(*AZURE_APP_SERVICE_RESOURCE_TYPES):
            for record in azure_facts(app).app_service_secret_references:
                ...
                findings.append(
                    self._finding_factory.build(
                        ...
                        rationale=(
                            f"{app.display_name} materializes the sensitive-classified setting "
                            f"{classification.normalized_name} as a literal App Service application setting. "
                            "Literal configuration can place credential material in Terraform plan/state and "
                            "App Service configuration; use a Key Vault reference or managed-identity-backed "
                            "secret retrieval instead. The literal value is intentionally excluded from this "
                            "finding."
                        ),
...
def _delivery_posture_evidence(record: Mapping[str, Any]) -> list[str]:
    ...
    return [
        ...
        f"setting_key={setting_name if isinstance(setting_name, str) and setting_name else 'unknown'}",
