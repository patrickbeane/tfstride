"""GCP firewall packet subsets shared by VPC rules and ordered policy layers.

Protocol numbers form a bounded set; ports stay as intervals. Layers consume
only their incoming traffic. Delegation skips the rest of one policy for its
matching subset. Unresolved constraints consume possible matches conservatively;
partial source CIDRs are not yet subtracted.
"""
...
class _Candidate:
    ...
    priority: int
    action: Literal["allow", "deny", "goto_next", "uncertain", "observe"]
    scope: PacketSubset
    match_path: str
...
@dataclass(frozen=True, slots=True)
class FirewallLayerResult:
    allowed: tuple[AllowedSubset, ...]
    continuing: tuple[PacketSubset, ...]
    uncertainties: tuple[str, ...]


def internet_packet_subsets() -> tuple[PacketSubset, ...]:
    return tuple(PacketSubset(family, _ALL_PROTOCOLS) for family in _INTERNET_SOURCES)


def evaluate_firewall_layer(
    firewalls: tuple[NormalizedResource, ...],
    incoming: tuple[PacketSubset, ...],
    *,
    policy: bool,
) -> FirewallLayerResult:
    if not incoming:
        return FirewallLayerResult((), (), ())
    candidates: list[_Candidate] = []
    uncertainties: set[str] = set()
    for resource in firewalls:
        for match in _firewall_matches(resource, policy=policy):
            if match["rule_direction"] == "egress" or match["rule_disabled"] is True:
                continue
            # Keep the existing VPC diagnostic contract. Policy diagnostics are
            # attached only to subsets that actually reach that rule.
            if not policy:
                uncertainties.update(f"{resource.address}: {reason}" for reason in match["uncertainties"])
            candidates.extend(_match_candidates(resource, match))

    undecided = list(incoming)
    delegated: list[PacketSubset] = []
    allowed: list[AllowedSubset] = []
    for candidate in sorted(candidates, key=_candidate_sort_key):
        # The implied VPC ingress deny wins ties at 65535. Policy priorities
        # have a separate range and must never inherit this VPC cutoff.
        if not policy and candidate.priority >= 65535:
            continue
        remaining: list[PacketSubset] = []
        for subset in undecided:
            ...
            if matched is None:
                continue
            uncertainties.update(candidate.uncertainties)
            if candidate.action == "allow":
                allowed.append(AllowedSubset(candidate, matched))
            elif candidate.action == "goto_next":
                delegated.append(matched)
            # Denies and unresolved constraints do not reach later rules.
        undecided = remaining
        if not undecided:
            break

    return FirewallLayerResult(tuple(allowed), tuple([*delegated, *undecided]), tuple(sorted(uncertainties)))


def firewall_ingress_decision(
    firewalls: tuple[NormalizedResource, ...],
    allowed: tuple[AllowedSubset, ...],
    ...
    return FirewallIngressDecision(sources=sources, uncertainties=tuple(sorted(set(uncertainties))))


def _candidate_sort_key(candidate: _Candidate) -> tuple[object, ...]:
    return (
        candidate.priority,
        {"deny": 0, "uncertain": 0, "observe": 1, "goto_next": 2, "allow": 3}[candidate.action],
...
def _match_candidates(resource: NormalizedResource, match: GcpFirewallMatch) -> list[_Candidate]:
    priority = match["rule_priority"] if match["rule_priority"] is not None else -1
    action = match["action"]
    issues = tuple(f"{resource.address}: {reason}" for reason in match["uncertainties"])
    candidates: list[_Candidate] = []
        ...
        for source in (*rule.cidr_blocks, *rule.ipv6_cidr_blocks):
            ...
            candidates.append(
                _Candidate(
                    resource.address,
                    priority,
                    action if action in ("allow", "deny", "goto_next") else "uncertain",
                    PacketSubset(
                        network.version,
    ...
    if action == "allow" and not issues:
        return candidates
    ...
    if unresolved_families and not issues:
        issues = (
    ...
    candidates.extend(
        _Candidate(
            resource.address,
            priority,
            "observe" if action == "allow" else "uncertain",
            PacketSubset(family, _protocols(match["protocol"]), start, end),
            match["path"],
            issues,
        )
...
def _consume_subset(subset: PacketSubset, scope: PacketSubset) -> tuple[PacketSubset | None, list[PacketSubset]]:
...
def _effective_ingress(allowed: tuple[AllowedSubset, ...]) -> list[GcpEffectiveFirewallIngress]:
...
def _firewall_matches(resource: NormalizedResource, *, policy: bool) -> list[GcpFirewallMatch]:
    ...
    if policy:
        return _legacy_policy_matches(resource)
...
def _legacy_policy_matches(resource: NormalizedResource) -> list[GcpFirewallMatch]:
    # Hand-built normalized resources express their known matches as generic
    # network rules. Keep this compatibility boundary out of precedence logic.
    matches: list[GcpFirewallMatch] = []
    for index, rule in enumerate(resource.network_rules):
        ports = None if rule.from_port is None or rule.to_port is None else [f"{rule.from_port}-{rule.to_port}"]
        records = normalize_firewall_matches(
            {
                "action": resource.get_metadata_field(GcpResourceMetadata.FIREWALL_POLICY_ACTION),
                GcpAttr.PRIORITY.key: resource.get_metadata_field(GcpResourceMetadata.FIREWALL_POLICY_PRIORITY),
                GcpAttr.DIRECTION.key: rule.direction,
                "match": [
                    {
                        "src_ip_ranges": [*rule.cidr_blocks, *rule.ipv6_cidr_blocks],
                        "layer4_configs": [{"ip_protocol": rule.protocol, "ports": ports}],
                    }
                ],
            },
            policy=True,
